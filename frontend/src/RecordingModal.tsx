...
import { ArrowUpRight, Check, Mic, Monitor, Pause, Play, Square, Upload, X } from 'lucide-react'
import { Modal, Spinner, Feedback } from './components'
...
export default function RecordingModal(props: Props) {
  ...
  if (!capture) return <Modal title="Record a meeting" onClose={props.onClose}><h2>New recording</h2>{error ? <><Feedback error={error} /><button className="button secondary" onClick={() => setBrowser(true)}>Use browser recording</button></> : <p className="capture-loading"><Spinner />Checking recording devices…</p>}</Modal>
...
function BrowserRecordingModal({ mode, settings, onClose, onSaved }: Props) {
  ...
  return <Modal title={mode === 'record' ? 'Record a meeting' : 'Import a recording'} onClose={requestClose} closeDisabled={saving}>
    <h2>{mode === 'record' ? 'New recording' : 'Import recording'}</h2>
    {discard ? <div className="discard-box"><strong>Discard this recording?</strong><p>Permanently deletes the unsaved audio.</p><div className="button-row"><button className="button secondary" onClick={() => setDiscard(false)}>Keep recording</button><button className="button danger" onClick={discardRecording}>Discard recording</button></div></div> : <>
    <Feedback error={error} />
    {!active && !retryRecording && <>
      <label className="field">Meeting title <span className="optional">optional</span><input placeholder="Untitled meeting" value={title} onChange={event => setTitle(event.target.value)} disabled={saving} maxLength={200} /></label>
      {mode === 'import' && <label className={`file-drop ${file ? 'selected' : ''}`} onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); if (!saving && event.dataTransfer.files[0]) setFile(event.dataTransfer.files[0]) }}><input type="file" accept="audio/*,video/*,.m4a,.mp3,.wav,.webm,.ogg,.flac,.mp4,.mkv" onChange={event => setFile(event.target.files?.[0] || null)} disabled={saving} /><span className="file-drop-icon">{file ? <Check size={24} /> : <Upload size={24} />}</span><strong>{file ? file.name : 'Choose or drop a file'}</strong><small>{file ? `${(file.size / 1024 / 1024).toFixed(1)} MB · click to change` : 'Audio or video · up to 2 GB'}</small></label>}
