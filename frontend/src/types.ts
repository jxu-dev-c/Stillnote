...
export type Meeting = {
  ...
  progress: number; stage: string; error: string | null; audio_name: string; audio_url: string;
  language: string; speaker_count: number | null; speakers: Record<string, string>; segments: Segment[];
  summary: Summary | null; notes: string;
}
...
export type Settings = { transcription: { model: string; language: string; speaker_count: number | null }; summary: { provider: 'local' | 'ollama' | 'openai-compatible' | 'anthropic'; model: string; base_url: string; api_key_set: boolean }; speech: SpeechStatus }
