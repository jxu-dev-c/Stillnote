...
struct CaptureTimeline {
    ...
    func seconds(at time: Double) -> Double { max(0, (pausedAt ?? time) - start - pausedDuration) }
...
final class PCMWriter {
    ...
    func append(_ buffer: AVAudioPCMBuffer, at seconds: Double) throws -> (Double, Double) {
    ...
    func close() throws { try header(); try file.synchronize(); try file.close() }
}
