...
- Saved audio lives in `data/audio/<id>`; screen video with the same mixed audio lives in `data/video/<id>`. Playback, seeking, speed controls, download, and meeting deletion handle both files. Video content is never supplied to transcription or summaries. Each recording's **Send video path to AI** toggle can include the local path as text in summary prompts; it defaults to off and does not enable video analysis.
