...
enum MossWhisperAudio {
    ...
    static func melFilters(nMels: Int) -> MLXArray {
        ...
        let filters = MLXAudioCore.melFilters(
            sampleRate: MossWhisperAudioConfig.sampleRate,
            nFft: MossWhisperAudioConfig.nFft,
            nMels: nMels,
            fMin: 0,
            fMax: Float(MossWhisperAudioConfig.sampleRate) / 2.0,
            norm: "slaney",
            melScale: .slaney
        )
    ...
    static func logMelSpectrogram(_ audio: MLXArray, nMels: Int) -> MLXArray {
        let nFft = MossWhisperAudioConfig.nFft
        let hop = MossWhisperAudioConfig.hopLength
