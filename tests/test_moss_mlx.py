import pytest
from meeting_app import moss_mlx, speech
...
def test_apple_silicon_selects_mlx_and_other_platforms_keep_torch(monkeypatch):
    monkeypatch.delenv("STILLNOTE_MOSS_BACKEND", raising=False)
    monkeypatch.setattr(speech.sys, "platform", "darwin")
    monkeypatch.setattr(speech.platform, "machine", lambda: "arm64")
    assert speech._moss_backend() == "mlx"
    monkeypatch.setattr(speech.platform, "machine", lambda: "x86_64")
    assert speech._moss_backend() == "torch"
    monkeypatch.setenv("STILLNOTE_MOSS_BACKEND", "mlx")
    assert speech._moss_backend() == "mlx"
    monkeypatch.setenv("STILLNOTE_MOSS_BACKEND", "remote")
    with pytest.raises(ValueError):
        speech._moss_backend()
